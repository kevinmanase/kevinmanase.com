...
a few days after the first run, i drove a real thread end-to-end against the optimized pack. two new agent turns landed cleanly through the v2 instructions: a `verify` move that produced a 702-character utterance, then another `verify` that produced a 430-character one. trace entries had `signatureName: "speak"` with valid enum values, zero parse failures, zero error logs. the boot path read `instructions_len: 3789` from `active.json` exactly as designed.

so the loop is not just compiled. it is shipped. PR merged into main, optimized prompt live in production for the next conversation that opens a thread.

next step is the autopilot. auto-trigger the compile when the trace gate trips again. gate auto-apply on `instructions_changed_from_baseline=true` plus a substantive-length check. hot-reload the running server. post-apply re-score so a regressing pack rolls itself back. that ticket is filed. when it lands, "stop hand-tuning prompts" stops being a slogan and starts being a cron job.
...
the closed loop is the win. traces in, optimized prompts out, fresh traces measure the lift, the next compile reads the receipts.
